...
The data stream is live, so the raw data products become available as soon as they are collected and transferred from Arizona to Illinois.
...
There are two ways to access data. To use search, select, and compute on data in the cloud sign up for Clowder. 
This is the easiest way to use large datasets, where you can select data and then open Rstudio and Jupyter interfaces.
To download data directly to your computer or server, use Globus.
Both of these options are described below.

## Clowder: Data Access and Online Computing
